...
resource "azurerm_snapshot" "data-disk1-snapshot" {
  count               = var.data_count
  name                = "${var.data_host_prefix}-snapshot-lun0-${count.index}"
  location            = var.location
  resource_group_name = var.es_rg
  create_option       = "Copy"
  source_uri          = azurerm_managed_disk.data-disk1[count.index].id
}
